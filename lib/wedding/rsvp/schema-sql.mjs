// Single source of truth for the wedding RSVP database schema.
//
// Plain ESM (no TypeScript) so it can be imported by BOTH the typed app/test
// code (lib/wedding/rsvp/migrations.ts) and the standalone migration runner
// (scripts/wedding-rsvp-migrate.mjs). Keeping one copy prevents the two from
// drifting — a drift here silently breaks reads when the app SELECTs a column
// the live database was never migrated to add.
//
// Rules:
// - `RSVP_SCHEMA_STATEMENTS` must be idempotent (CREATE TABLE/INDEX IF NOT
//   EXISTS) — it runs on every migrate.
// - To add a column to an existing table, append an ALTER to
//   `RSVP_SCHEMA_ALTER_STATEMENTS` AND add it to the CREATE TABLE above, so
//   fresh and existing databases converge. Duplicate-column errors are ignored.

export const RSVP_SCHEMA_STATEMENTS = [
...
export const RSVP_SCHEMA_ALTER_STATEMENTS = [
